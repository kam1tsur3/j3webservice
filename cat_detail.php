...
<?php
	if(isset($_SESSION['login_name'])){
		...
		$result = pg_execute($conn, "q1", array($c_id));
		$arr = pg_fetch_assoc($result);
print<<<EOS
		{$arr['name']}<br>
		<img src="{$arr['img']}" width="400" height="400"><br>
		指名回数：{$arr['rsvd_cnt']}回<br> 
		ご指名はこちらから<br>
		<form action="./reservation.php" method="post">
		来店予定日付：
		<input type="date" name="rsv" value="<?=date('Y-m-d')?>" 
		min="<?=date('Y-m-d')?>"/><br>
		<input type="hidden" name="c_id" value="{$c_id}"><br>
		<input type="submit" value="指名する"><br>
		</form>
EOS;
	}else{
