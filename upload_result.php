...
if(isset($_SESSION['login_name'])){
	...
	$query_test = "select * from cats where name = $1";
	$result_test = pg_prepare($conn, "test", $query_test);
	$result_test = pg_execute($conn, "test", array($cat_name));
	
	if(pg_num_rows($result_test)==0){
		$query = "insert into cats (name, rsvd_cnt, img) values($1, 0, $2)";
		$result = pg_prepare($conn, "upload", $query);
		$uploaddir = "./cimages/";
		$uploadfile = $uploaddir . $_SESSION['id']."_".basename($_FILES['upfile']['name']);
		if(is_uploaded_file($_FILES['upfile']['tmp_name'])){
			if(move_uploaded_file($_FILES['upfile']['tmp_name'], $uploadfile)){
				$result = pg_execute($conn, "upload", array($cat_name, $uploadfile));
				print "登録が完了しました<br>";
			}
		} 
		else{
			$default_pass = './images/default.png';
			$result = pg_execute($conn, "upload", array($cat_name, $default_pass));
		}
	}
	else{
		print "その名前のキャストがすでに在籍します。名前を変更してください。<br>";
	}
}
else{
	print "キャストの応募をするには会員登録が必要です。<br>";
}
?>
